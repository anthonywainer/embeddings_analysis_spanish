...
class GensimEmbedding(Logger):
    ...
    * GLOVE
    ...
    def extract_gensim_embedding(self, embedding_name: str,
                                 texts: np.array,
                                 max_len: int,
                                 vector_embedding_path: str
                                 ) -> np.ndarray:
        """
        Method to extract embeddings from KeyedVectors
        :param vector_embedding_path: Vector Path
        ...
        if embedding_name not in self.locals:
            self.logger.info(f"Loading {vector_embedding_path}")
            self.locals[embedding_name] = KeyedVectors.load_word2vec_format(vector_embedding_path)
