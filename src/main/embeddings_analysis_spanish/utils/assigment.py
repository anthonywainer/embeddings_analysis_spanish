...
# Heavily refactored by Anthony Cachay
...
def linear_assignment(values: np.ndarray) -> ndarray:
    ...
    References
    ----------
    1. Harold W. Kuhn. The Hungarian Method for the assignment issue.
       *Naval Research Logistics Quarterly*, 2:83-97, 1955.
    2. Harold W. Kuhn. Variants of the Hungarian method for assignment issues.
    ...
    :param values: The cost matrix of the bipartite graph
    :return indices :
        The pairs of (row, col) indices in the original array giving
        the original ordering.
    """
    indices = _hungarian(values).tolist()
    indices.sort()
...
class _HungarianState:
    ...
    def __init__(self, cost_matrix: np.ndarray) -> None:
        ...
        :param cost_matrix: The cost matrix. Does not have to be square.
        """
...
def _hungarian(cost_matrix: np.ndarray) -> np.array:
...
def _step1(state: np.array) -> np.array:
...
def _step3(state: np.array) -> np.array:
    """
...
def _step4(state: np.array) -> np.array:
    """
...
def _step5(state: np.array) -> np.array:
    """
...
def _step6(state: np.array) -> np.array:
    """
    ...
    if np.any(state.row_uncovered) and np.any(state.col_uncovered):
        min_val = np.min(state.C[state.row_uncovered], axis=0)
        min_val = np.min(min_val[state.col_uncovered])
        state.C[np.logical_not(state.row_uncovered)] += min_val
        state.C[:, state.col_uncovered] -= min_val
    return _step4
